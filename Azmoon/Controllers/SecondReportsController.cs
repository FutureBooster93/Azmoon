...
namespace Azmoon.Controllers
{
    public class SecondReportsController : Controller
    {
        ...
        public SecondReportsController(ApplicationDbContext _db)
        {
            db = _db;
        }
        ...
        {
            SecondReportsViewModel viewModel = new SecondReportsViewModel();
        ...
        public IActionResult PreReport(SecondReportsViewModel viewModel)
        {
            ...
            foreach (var column in viewModel.selectedColumns)
            {
                if (SD.allColumns.Contains(column))
                {
                    ...
                    {
                        case "id":
                            var ids = db.TblApplicants1.Select(a => a.Id).ToList();
                            ...
                            break;
                        case "firstname":
                            var firstNames = db.TblApplicants1.Select(a => a.FirstName).ToList();
                            ...
                            break;
                        case "lastname":
                            var lastNames = db.TblApplicants1.Select(a => a.LastName).ToList();
                            ...
                            break;
                        case "postalcode":
                            var postalCodes = db.TblApplicants1.Select(a => a.PostalCode).ToList();
                            ...
                            break;
                        case "tel":
                            var tels = db.TblApplicants1.Select(a => a.Tel).ToList();
                            ...
                            break;
                        case "email":
                            var emails = db.TblApplicants1.Select(a => a.Email).ToList();
